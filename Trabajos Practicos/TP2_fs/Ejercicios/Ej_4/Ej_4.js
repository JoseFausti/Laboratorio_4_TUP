...
function countWordInFile(fileName, word) {
    ...
    const wordCount = data.match(new RegExp(word, 'gi')).length; // new RegExp(word, 'gi') crea una expresión regular para buscar la palabra en el texto. 'gi' significa que se buscan todas las apariciones (g de global) y la búsqueda es insensible a mayúsculas y minúsculas (i de insensitive).
    console.log(`La palabra "${word}" aparece ${wordCount} veces en el archivo "${fileName}".`);
}

// Ejecutar la función
countWordInFile('archivo.txt', 'palabras');